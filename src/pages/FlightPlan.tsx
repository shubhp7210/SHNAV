...
const FlightPlan = () => {
  ...
  const runATMEngines = async (intentId: string | null, tScore: number, wRisk: string, cCount: number, rData: RouteOptimizerResult | null) => {
    ...
    try {
      const depStart = data.departureWindowStart
        ? `${data.departureDate || new Date().toISOString().split("T")[0]}T${data.departureWindowStart}:00`
      ...
      const [weatherRes, airspaceRes, vertiportRes, trajectoryRes] = await Promise.allSettled([
        ...
        supabase.functions.invoke("airspace-scheduler", {
          body: {
            ...
            departure_window_end: `${data.departureDate || new Date().toISOString().split("T")[0]}T${data.departureWindowEnd}:00`,
          },
        }),
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="sticky bottom-0 z-30 border-t border-border/50 bg-card/80 backdrop-blur-xl">
        <div className="container max-w-3xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3" style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom, 0px))" }}>
          <button
            onClick={goBack}
